...
pub async fn get_pool() -> Result<Pool, DatabaseError> {
...
pub async fn get_collectors(pool: &Pool) -> Result<Vec<CollectorDB>, DatabaseError> {
    Ok(sqlx::query_as!(CollectorDB, "select * from collectors")
        .fetch_all(pool)
        .await?)
}

pub async fn get_collector_by_id(pool: &Pool, id: i32) -> Result<CollectorDB, DatabaseError> {
    Ok(
...
pub async fn get_collector_metrics(pool: &Pool, id: i32) -> Result<Vec<Metrics>, DatabaseError> {
